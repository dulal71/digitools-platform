...
<p className='mt-3'>Choose from our curated collection of premium digital products designed <br /> to boost your productivity and creativity.</p>
</div>
...
<div className="tabs tabs-box flex gap-5">
  ...
  <input onClick={()=>setOpen('cart')} type="radio" name="my_tabs_1" className={`tab ${open==='cart' ?'rounded-full px-12 bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white':'text-gray-600'} `} aria-label="Cart ()" />
</div>
</div>
            </div>
         {open === 'Products'? <Products data={data} ></Products> : <Cart></Cart>}   
        </div>
    );
};
